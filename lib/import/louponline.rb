class Import::Louponline < Import::Platform::Shopify

  # platform = shopify

  def baseurl; 'http://www.louponline.com'; end
  def brand_name; 'Loup'; end
  ...
  def process_product(url)
    ...
    js = orig_js.match(/jQuery\(document\)\.ready\(function\(\$\)\s?\{\s+var optionSelectors = (new Shopify\.OptionSelectors\("product-select", {.*}\);).*\}\);/m)[1]
