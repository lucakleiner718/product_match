class Parser::Dvf
	...
	def process url
		...
		product_name = html.css('.product-name').text
		...
		colors.each do |color|
			...
			color_link = "http://www.dvf.com/on/demandware.store/Sites-DvF_US-Site/default/Product-Variation?pid=#{product_id.sub('+', '%2B')}&dwvar_#{param_product_id}_color=#{color_id}&format=ajax"
			...
			if sizes.size > 0
				...
				# end
					...
					link = "http://www.dvf.com/on/demandware.store/Sites-DvF_US-Site/default/Product-Variation?pid=#{product_id.sub('+', '%2B')}&dwvar_#{param_product_id}_size=#{size_value}&dwvar_#{param_product_id}_color=#{color_id}&format=ajax"
					puts link
	...
	def save_data data
		...
		filename = "tmp/dvf-#{Time.now.to_i}.csv"
